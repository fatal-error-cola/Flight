...
RouteMenuItem::RouteMenuItem(size_t index_, QWidget *parent):
...
void RouteMenuItem::setUi() {
	...
	auto *flight = new QLabel(route.info.flight);
	flight->setFont(QFont("Arial", 30, QFont::Bold));
	layout->addWidget(flight, 0, 0, -1, 1);
	...
	connect(delete_button, &QPushButton::clicked,
		[this]() { Routes::getInstance()->remove(index); deleteLater(); });
}
